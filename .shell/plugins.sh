...
if grep -q "[update_manager $1]" ${MOD_CONF}/moonraker.conf || grep -q "[update_manager $1]" ${MOD_CONF}/mod_data/user.moonraker.conf; then
    ...
    if [ "$2" == "Enable" ]; then
        ...
        [ -x "${MOD_CONF}/mod_data/plugins/$1/install.sh" ] && "${MOD_CONF}/mod_data/plugins/$1/install.sh"
    else
        if [ "${ZLANG}" != 'ru' ]; then
        ...
        sed -i "/plugins\/$1\//d" ${MOD_CONF}/mod_data/plugins.cfg
        [ -x "${MOD_CONF}/mod_data/plugins/$1/uninstall.sh" ] && "${MOD_CONF}/mod_data/plugins/$1/uninstall.sh"
    fi
